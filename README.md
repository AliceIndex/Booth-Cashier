...
---
## ⚠️ セキュリティに関する重要なお願い (ネットワーク環境について)

本システムは保守性を重視し、通信の暗号化（HTTPS）や複雑なログイン認証を省いたシンプルな設計になっています。
**コミケ等のイベント会場で提供されているフリーWi-Fiや、不特定多数が接続できる共有ネットワーク上では絶対に起動・利用しないでください。**
売上データや商品情報の盗聴、第三者による不正な操作の原因となります。

**【推奨する安全な運用方法】**
必ず、以下のいずれかの「閉じたネットワーク」で運用してください。
1. **Windowsのモバイルホットスポット機能**を利用し、サーバーPC自身をWi-Fi親機にする（最も手軽で推奨）
2. サークル関係者しかパスワードを知らない専用のモバイルルーターを使用する
3. 担当者のスマートフォンのテザリング機能を使用する
